...
timestamped run, keeps the source files untouched, and establishes the incumbent on a 50-game
holdout. Start with a controlled 500k validation run before committing to another overnight run:
...
rl\venv\Scripts\python.exe -m rl.train `
    ...
    --champion-eval-episodes 50 `
    --champion-max-eval-episodes 200 `
    --n-step 3 `
    --priority-alpha 0.6 `
    --device cuda `
    --seed 42 `
...
The first 50k transitions are generated by the frozen incumbent and kept in a protected replay
slice; 25% of every later batch remains anchored to that behavior. Updating every fourth decision
reduces destructive over-training and is also substantially faster. Three-step returns propagate
brick/death consequences sooner, while proportional prioritized replay (`alpha=0.6`) revisits large
TD-error transitions; importance weights anneal from beta `0.4` toward `1.0`. Ten-game evaluations every 10k
steps are diagnostics and candidate nominations only. A positive result on the initial 50 unseen,
paired seeds always expands to 200 games, including candidates that already look decisive at 50.
Promotion requires the final 200-game paired 95% confidence lower bound to exceed 10 points. Attempts
and rejections are recorded in `gates.csv`; a still-positive rejection is saved under `nominees/` for
later audit. Only a candidate accepted after the full confirmation replaces `best.pt` or is published
by `--live-export`. `latest.pt` is never assumed to be the champion.

Reward shaping remains available for deliberate experiments via `--paddle-hit-reward` and the paired
`--stall-paddle-hits` / `--stall-penalty` flags, but it is disabled in the stability-first command
above. Changing reward semantics on resume requires `--fork-run`. The optional
`--adaptive-patience-steps N` plateau controller is also available, but should be added only after a
500k anchored branch demonstrates stable holdout improvement. Checkpointed settings are inherited on
ordinary `latest.pt` resume after an interruption.

### Breaking a level-one plateau (fresh curriculum run)

Fine-tuning a converged champion tends to find nothing new. To teach the agent past a stuck
single-life score, change the *learning problem* on a **fresh** run (no `--resume`):

- `--curriculum-clear-max 0.6` — train only: at each reset, pre-clear a uniform-random fraction (up
  to 60%) of the wall and advance the ball speed to match, so the buffer contains fast-ball, sparse
  endgame states instead of only full-board starts. Evaluation always uses the full board, so
  `eval_score_mean` stays comparable to earlier runs.
- `--curriculum-prob 0.5` — train only: **required with the curriculum.** Only apply the pre-clear on
  this fraction of resets; the rest stay full-board openings. Without it (the `1.0` default) ~96% of
  episodes start mid-game and the agent never practises — or, when fine-tuning, *forgets* — the slow
  opening that eval measures. Empirically `1.0` fails; `~0.5` is the fix that keeps both skills.
- `--gamma 0.997` — lengthen the credit horizon (~3 s at the default `0.99` to ~11 s), so the value
  function can represent long survival and the level-clear bonus stops being invisible.
- `--epsilon-decay-steps 500000` — spend more of the run exploring before the 0.05 floor.

```powershell
rl\venv\Scripts\python.exe -m rl.train `
    --steps 1000000 `
    --curriculum-clear-max 0.6 `
    --curriculum-prob 0.5 `
    --gamma 0.997 `
    --n-step 3 `
...
The same mix curriculum also works as champion **fine-tuning** — fork the champion and add
`--curriculum-clear-max 0.4 --curriculum-prob 0.5` with a low LR and the anchor, so it gains endgame
coverage without forgetting the opening (a fork with `--curriculum-prob 1.0` degraded the champion to
~400 because it starved the opening — keep the mix):

```powershell
rl\venv\Scripts\python.exe -m rl.train `
    --resume rl\runs\20260720-125627\best.pt --fork-run `
    --steps 500000 `
    --curriculum-clear-max 0.4 --curriculum-prob 0.5 `
    --learning-rate 0.00003 --reset-optimizer `
    --learn-every 2 --anchor-steps 50000 --anchor-fraction 0.25 `
    --n-step 3 --priority-alpha 0.6 --gradient-clip-norm 10 `
    --device cuda --seed 42
```

`--curriculum-clear-max`, `--curriculum-prob`, and `--gamma` change run semantics, so altering any on
an ordinary resume is rejected and needs `--fork-run`; `--epsilon-decay-steps` is a forward-only
schedule and can be overridden freely. No `--live-export` on experiments — the experimental policy
must not overwrite the deployed champion. When the run finishes, decide promotion with the paired A/B
audit, which reuses the trainer's own 95%-confidence gate:
...
Only publish (via `rl.export_policy`) on a `PROMOTE` verdict.
